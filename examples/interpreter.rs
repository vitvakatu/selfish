...
fn eval(mut s: LispValue, mut env: Environment) -> LispResult {
    loop {
        match **s.clone() {
            LispType::List(ref v) => {
                if v.len() == 0 {
                ...
                match **v[0] {
                    ...
                    LispType::Symbol(ref sym) if sym == "fn" => {
                        ...
                        let body = (**v[2]).clone();
                        return Ok(LispValue::closure(binds, LispValue::new(body), env.clone()));
                    }
                    _ => {
                        ...
                        match **evaluated {
                            LispType::List(ref v) => {
                                let args = v[1..].to_vec();
                                match **v[0].clone() {
                                    LispType::Func(func) => return func(args),
